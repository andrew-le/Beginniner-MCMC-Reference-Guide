...
num_sim <- 1000
theta_samps <- numeric(num_sim)

theta_curr <- rgamma(1, shape = 3, rate = 1) #initialize theta
...
for (t in 1:num_sim) {
  ...
  if (log_ratio >= log(runif(1))) {
    theta_samps[t] <- theta_prop
    theta_curr <- theta_prop #update the theta_curr with the proposed value
  } else{
    theta_samps[t] <- theta_curr
  }
}
```
...
hist(theta_samps, freq = FALSE, xlab = "", ylab = "", breaks = 100,
